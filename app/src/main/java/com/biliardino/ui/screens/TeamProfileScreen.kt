...
@Composable
fun TeamProfileScreen(teamId: Long, name: String, s: UiState, vm: AppViewModel) {
    val profile = s.currentTeamProfile
    ...
    if (profile == null) {
        Box(Modifier.fillMaxSize().padding(32.dp), contentAlignment = Alignment.Center) {
            Text("Impossibile caricare il profilo della squadra $name", textAlign = TextAlign.Center)
        }
        return
    }
    ...
    LazyColumn(
        ...
        item {
            ProfileHeader(name = profile.teamName, icon = Icons.Default.Groups)
        }
